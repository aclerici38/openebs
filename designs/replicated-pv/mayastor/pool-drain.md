...
**Pre-requisites:** the pool must not already be in a `Queued`, `Draining`, or
...
- **Abort/cleanup reconciler.** Driven by `DrainPhase: Aborted`, and purely an orchestrator: it
  propagates `unwind = Some(Abort)` into each volume's runtime `replica_move` slot, waits for the
  delegated unwind to signal completion via `spare_replica → None`, clears the markers, then clears
...
spare — recognised by the owning volume's marker carrying `spare_replica == that id`. We will attempt
to remove the `spare_replica` here and set `spare_replica` to None so that we attempt to schedule it again.
If we wait on it, it will get evicted once it becomes online from the Pool B's drain anyway and its a wasted
